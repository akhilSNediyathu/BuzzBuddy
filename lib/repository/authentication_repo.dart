...
static Future <Response?> verifyOtp(String email, String otp)async{
 ...
 try {
  ...
  static Future<Response?> verifyOtpPasswordReset (String email, String otp)async{
 try {
  ...
  static Future<Response?> googleLogin (String email)async{
    try {
      ...
      var response = await client.post(Uri.parse(ApiEndpoints.baseUrl+ApiEndpoints.googleLogin),body: jsonEncode(finalEmail),headers: {"Content-Type": 'application/json'});
